...
for class_idx,class_name in enumerate(qualified['class'].unique()):
    ...
    for vid_idx,vid_file in enumerate(video_paths):
        ...
        data_frame = torch.zeros(0)
        for frame_idx in random_idx:
            # get frame
            ...
            data_frame = torch.cat((data_frame, frame))
        # augmentation and transformation
        data[vid_idx] = data_frame.permute((1,0,2,3))
    ...
    with torch.no_grad():
        ...
        for vid_idx in range(len(video_paths)):
            input_tensor = data[vid_idx].unsqueeze(0).cuda()#to(device)
