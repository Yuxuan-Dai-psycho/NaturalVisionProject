...
info = pd.DataFrame(columns=['network', 'scaler', 'model', 'C', 'percentile', 
                             'acc_cv', 'acc_test_single', 'acc_test_mean'])

network_names = {'secondary':[2], 'all':[1,2,10,11]}
...
for net in network_names.keys():
    ...
    for scaler_name in scalers.keys():
        ...
        #% =======Enhance data====================
        # ======== remove outlier=================
        # out_index = find_outlier(data, 0.05)
        ...
        train_idx = [32, 38, 18, 4, 3, 21, 8, 9, 10, 28, 17, 29, 27, 2, 20, 13, 
                     33, 6, 19, 36, 22, 26, 7, 0, 12, 34, 14, 25, 37, 23] #random.sample(range(n_run), n_train)
        ...
        param_grid = [
                      {'classifier': [LinearSVC(max_iter=8000)], 'feature_selection':[SelectPercentile()],
                       ...
                       'feature_selection__percentile': [5, 10, 20,50],},
        ...
        # save info
        info.loc[loop_idx, ['network', 'scaler', 'model', 'C', 'percentile', 
                            'acc_cv', 'acc_test_single', 'acc_test_mean']] = \
            net, scaler_name, type(best_params['classifier']).__name__, best_params['classifier'].C, \
            best_params['feature_selection'].percentile, \
...
info.to_csv(f'{out_path}/grid_search_{sub_name}.csv', index=False)
